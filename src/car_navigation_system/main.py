...
class SimpleDrivingSystem:
    ...
    def draw_ar_navigation(self, display_img):
        ...
        try:
            ...
            # 获取前方一定距离的下一个路点
            next_waypoints = current_waypoint.next(10.0)  # 10米前方
            if not next_waypoints:
                return display_img
            ...
            center_y = img_height - 150  # 在画面底部中央

            # 箭头颜色（根据转向角度变化）
            if abs(relative_yaw) < 15:
                arrow_color = (0, 255, 0)  # 绿色 - 直行
            elif abs(relative_yaw) < 45:
                arrow_color = (0, 255, 255)  # 黄色 - 轻微转向
            else:
                arrow_color = (0, 165, 255)  # 橙色 - 大角度转向
            ...
            # 根据转向角度计算箭头位置
            offset_x = int(relative_yaw * 3)  # 横向偏移，最大约90像素
